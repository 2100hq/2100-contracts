...
contract Controller is Claimer, Minter, Creator{
    ...
    function deposit(uint256 _amount) notOwner public returns (bool) {
        ...
        require(DAI.allowance(msg.sender, address(this)) >= amount, "2100: Controller does not have enough allowance granted");
        require(DAI.balanceOf(msg.sender) >= amount, "2100: Sender does not have sufficient balance to deposit");
        require(DAI.balanceOf(msg.sender) > 0, "2100: Sender balance cannot be zero");

        require(DAI.transferFrom(msg.sender, address(this), amount), "2100: DAI transfer failed");
